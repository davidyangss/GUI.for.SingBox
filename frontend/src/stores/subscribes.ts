  ...
  const _doUpdateSub = async (s: Subscription, options: Partial<Subscription> = {}) => {
    ...
    if (s.type === 'Http') {
      ...
      const requestHeaders = {
        'User-Agent': s.header.request['User-Agent'] || options.header?.request?.['User-Agent'] || defaultUA,
        ...s.header.request,
    ...
    // Built-in format conversion (runs before plugin trigger)
    try {
    ...
    if (proxies.some((proxy) => proxy.name && !proxy.tag) || proxies[0]?.base64) {
      console.warn('[Subscribes] Non-sing-box format detected after conversion, plugin may handle conversion')
    }
